            ...
            if (Error_enter==False):
                if E_data:
                    ...
                    if uploadfile_e:
                        ...
                        exp_data().append({"Experiment_number":Experiment,"Experiment_name":Experiment_name,"SALUS_start":SALUS_start,"SALUS_end":SALUS_end, "start_doy":start_doy,\
                        "wx_id":wx_id, "soilID_list":soilID_list})
                        if write_data:
                            pd.DataFrame(exp_data()).to_csv('Experiment_file.csv')
                ...
                if M_data: 
                    if uploadfile_m:
                        ...
                        if write_data:
                            dataframe_e.to_csv('Management_file_createdfromfile.csv')
                    else:
                        ...
                        if spinup =='Yes':
                            for i in range(Years_spinup):
                                ...
                                get_data().append({"Experiment_number":Experiment,"crop_name":crop_name, "crop_mode": crop_mode, "speciesID":speciesID,\
                                "cultivar": cultivar,"plant_depth":plant_depth,"yearp":yearp,"ppop":ppop ,"rowspc":rowspc,\
                            ...
                            get_data().append({"Experiment_number":Experiment,"crop_name":crop_name, "crop_mode": crop_mode, "speciesID":speciesID,\
                                "cultivar": cultivar,"plant_depth":plant_depth,"yearp":yearp,"ppop":ppop ,"rowspc":rowspc,\
                                    ...
                                    "yearh":yearh, "hdoy":hdoy,"hcom":hcom,"hpc":hpc,"knock_down":knock_down,"hbpc":hbpc})
                        if write_data:
                            pd.DataFrame(get_data()).to_csv('Management_file.csv')
            else:
